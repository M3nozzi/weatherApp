...
class WeatherManager {
    func getCurrentWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ResponseBody {
        ...
        guard let infoDictionary: [String: Any] = Bundle.main.infoDictionary else {
            throw NSError(domain: "MissingInfoDictionary", code: 1, userInfo: nil)
        }
        guard let myWeatherApiKey: String = infoDictionary["MyWeatherApiKey"] as? String else {
            throw NSError(domain: "MissingAPIKey", code: 2, userInfo: nil)
        }
